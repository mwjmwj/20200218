...
//请求后台开奖数据
//http://localhost:8080/six-api/smallSix/testCurrentVideoInfo.do
...
function loadAjax_old () {
    ...
    $.ajax({
        type: "get",
        url:"read3.asp?rannum="+Math.ceil(Math.random()*1000000+1000),
        //url: "//1680660.com/smallSix/findCurrentVideoInfo.do?",
//      url: "//192.168.1.108:8080/six-api/smallSix/findCurrentVideoInfo.do?",
//      url:"http://192.168.1.140/six-api/smallSix/findCurrentVideoInfo.do",
        async: true,
...
function kaijiIn(maArr) {
    if(In == "") {
        ...
        $("#die").addClass("play");
        $(".status").css("background-image", "url(img/sssq/status.gif)")
        var eachnone = setTimeout(function() {
            $(".each").css("display", "none")
            //$("#die").css("background-image", "initial");
            $("#die").css("cssText","background-image:url(img/sssq/empt.png) !important");
            clearTimeout(eachnone)
        }, 1000)
        var eachtime = setTimeout(function() {
            $(".each").css("display", "block").css("background-image", "url(img/each.gif)");
...
function nextopenTime(nextTime) {
    ...
    nexttimeinter = setInterval(function() {
        ...
        if(day < 0 || hours < 0 || fen < 0 || miao < 0) {}
            ...
            var textime = '距离下期开奖还有：' +day + '天' + hours + '小時' + fen + '分鍾' + miao + '秒'
            $("#nextTime").text(textime)
        }
    }, 1000)
}
